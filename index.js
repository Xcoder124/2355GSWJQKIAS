...
app.post('/api/claim-birthday-bonus', authenticateToken, async (req, res) => {
    const userId = req.user.uid;
    const userEmail = req.user.email; // Available from decodedToken
    ...
    try {
        const result = await dbAdmin.runTransaction(async (transaction) => {
            ...
            // Ensure 'birthdayDate' is stored in user's Firestore document.
            // For this example, we'll assume 'birthdayDate' is a string in 'MM-DD' format.
            // If it's a Firebase Timestamp, you'll need to convert it.
            if (!userData.birthdayDate || typeof userData.birthdayDate !== 'string') {
                // If birthdayDate is a Timestamp:
                // if (!userData.birthdayDate || !userData.birthdayDate.toDate) {
                throw new Error("Birthday information not found or in unexpected format for user.");
            }

            const today = new Date();
            const currentMonth = String(today.getMonth() + 1).padStart(2, '0');
            const currentDay = String(today.getDate()).padStart(2, '0');
            const currentDateMMDD = `${currentMonth}-${currentDay}`;
            const currentYear = today.getFullYear();

            let userBirthdayMMDD = userData.birthdayDate;
            // Example: if userData.birthdayDate is a Firestore Timestamp object:
            // const userBirthDateFromTimestamp = userData.birthdayDate.toDate();
            // const birthMonth = String(userBirthDateFromTimestamp.getMonth() + 1).padStart(2, '0');
            // const birthDay = String(userBirthDateFromTimestamp.getDate()).padStart(2, '0');
            // userBirthdayMMDD = `${birthMonth}-${birthDay}`;


            if (userBirthdayMMDD !== currentDateMMDD) {
                throw new Error("Verification failed: Today is not your birthday according to server.");
            }

            // Check if bonus already claimed this year
            const claimedYearField = `birthdayBonusClaimed_${currentYear}`;
            if (userData[claimedYearField] === true) {
                throw new Error(`Birthday bonus for ${currentYear} has already been claimed.`);
            }
            ...
            const userUpdates = {
                balance: FieldValueAdmin.increment(bonusAmount),
                [claimedYearField]: true,
                lastTransaction: FieldValueAdmin.serverTimestamp(),
                [`transactionStats.${TransactionTypeServer.BirthdayBonus}`]: FieldValueAdmin.increment(1)
            };
            if (!userData.firstTransaction) { // Set firstTransaction if it's their first ever
                userUpdates.firstTransaction = FieldValueAdmin.serverTimestamp();
            }
            if (!userData.transactionCount || userData.transactionCount === 0) {
                 userUpdates.transactionCount = FieldValueAdmin.increment(1);
            } else {
                 userUpdates.transactionCount = FieldValueAdmin.increment(1);
            }
            ...
            const transactionData = {
                timestamp: FieldValueAdmin.serverTimestamp(),
                type: TransactionTypeServer.BirthdayBonus,
                amount: bonusAmount,
                reference: transactionRef.id,
                description: `Happy Birthday! Claimed ${bonusAmount} bonus points.`,
                externalReference: `BIRTHDAY-${currentYear}-${userId}`,
                relatedDocId: userId, // Relate to the user document itself
                metadata: {
                    reason: "Annual Birthday Bonus",
                    year: currentYear,
                    claimedBy: userEmail
                }
            };
            ...
            const newBalance = (Number(userData.balance) || 0) + bonusAmount;

            return {
                message: `Happy Birthday! ${bonusAmount} points added to your account.`,
        ...
        console.error(`SERVER ERROR /api/claim-birthday-bonus for user ${userId}:`, error);
        // Send a more specific error message if it's one of our thrown errors
        res.status(error.message.includes("Verification failed") || error.message.includes("already claimed") || error.message.includes("not found") ? 400 : 500)
           .json({ success: false, error: error.message || "Failed to claim birthday bonus." });
    }
});
